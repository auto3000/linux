...
// Temp: Reset on Odroid C2 is GPIOX.21 => gpio491
	cs4245->reset_gpio = devm_gpiod_get(&i2c_client->dev, "reset", GPIOD_OUT_LOW);
	if (IS_ERR(cs4245->reset_gpio)) {
		dev_err(&i2c_client->dev, "Error cs4245->reset_gpio = %d\n", 
			cs4245->reset_gpio);
		return PTR_ERR(cs4245->reset_gpio);
	}
	if (cs4245->reset_gpio) {
		mdelay(1);
		gpiod_set_value(cs4245->reset_gpio, 0);
		dev_info(&i2c_client->dev, "gpio is active low = %d\n",
			 gpiod_is_active_low(cs4245->reset_gpio));
	}
